...
**⚠️ 알려진 이슈: 커스텀 capability에 명령을 나중에 추가하면 한동안 불안정합니다.** `send`(현재는 `replayLearnedCode`로 개명) 명령을 capability에 추가한 뒤 실기기로 확인해보니:

- 한동안 클라우드가 "OO is not a valid value"로 명령 자체를 거부합니다 (요청마다 다른 백엔드 서버로 라우팅되는 듯, 거부/수락이 요청마다 뒤바뀝니다).
- 클라우드가 처음 수락하는 바로 그 전환 시점에는, 허브로 전달되는 메시지 자체가 깨져서 드라이버가 아닌 **SmartThings 프레임워크 자체**(`st/driver.lua`)가 `json.decode` 중 에러를 냅니다 (`bad argument #1: error converting Lua nil to String`). 우리 코드(`init.lua`)에는 도달하지도 못하는 단계라 드라이버 쪽에서 고칠 방법이 없습니다.
- 명령 이름을 바꿔봐도(`send` → `replayLearnedCode`) 똑같은 패턴이 재현되는 걸 확인했습니다 — 즉 특정 이름의 문제가 아니라, **새 capability 명령이 SmartThings 백엔드 전체에 고르게 전파되는 데 걸리는 시간** 자체의 문제입니다.
- `learn`/`cancelLearn`/`sendCode`(capability 생성 시점부터 있던 명령들)는 이런 문제가 전혀 없이 항상 안정적으로 동작합니다.
