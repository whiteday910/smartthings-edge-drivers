...
커스텀 capability `acrosswatch58328.irBlasterV4` 하나로 동작하며, 아래 5가지 항목이 "버튼 0"~"버튼 9" 10개 컴포넌트 각각에 동일하게 반복됩니다 (자세한 다중 신호 구조는 아래 "여러 개의 신호 학습하기" 참고). 가장 자주 쓰는 "저장된 신호 재생"이 맨 위에 오도록 배치했습니다:

| 화면 표시 | 종류 | 설명 |
|---|---|---|
| 저장된 신호 재생 | 버튼 (`replayLearnedCode`) | 가장 최근 학습한 신호를 그대로 재전송 |
| 저장된 신호 | 상태 텍스트 | `없음` / `저장됨` — 재생 가능한 코드가 있는지 한눈에 확인 |
| 새 신호 학습 | 버튼 (`learn`) | 학습 모드로 전환. 이후 몇 초 안에 기존 리모컨을 기기에 가까이 대고 버튼을 누르면 신호가 저장됩니다. 60초 안에 신호가 안 들어오면 자동으로 학습이 취소됩니다 |
| 학습 취소 | 버튼 (`cancelLearn`) | 신호를 저장하지 않고 학습 모드 종료 |
| 학습 상태 | 상태 텍스트 | `대기` / `학습 중` |

기기 목록(대시보드) 카드에도 "저장된 신호" 상태가 바로 보이고, 카드에서 바로 누를 수 있는 재생 버튼이 있어서 상세 화면을 열지 않고도 "버튼 0"의 신호를 재생할 수 있습니다.
...
기기를 여러 개로 쪼개는 대신(자식 기기 방식은 시도해봤지만 원하신 방식이 아니었습니다), **SmartThings의 "컴포넌트" 기능**을 사용했습니다 — 기기 하나에 여러 컴포넌트를 두면, 앱이 기기 상세 화면 안에서 컴포넌트를 탭/구간 형태로 전환할 수 있게 보여줍니다. 이 드라이버는 메인 기기 화면 안에 **"버튼 0"부터 "버튼 9"까지 10개의 컴포넌트**를 두었고, 각 컴포넌트가 완전히 독립적인 학습 상태/저장된 신호/버튼 세트를 가집니다 (마치 TV 리모컨의 숫자 버튼처럼, 0번은 "TV 전원", 3번은 "에어컨 켜기" 하는 식으로 각각 학습시켜 쓸 수 있습니다).

- 화면 위쪽의 "버튼 0" ~ "버튼 9" 중 원하는 번호를 선택하면, 그 번호 전용 저장된 신호 재생 / 저장된 신호 / 새 신호 학습 / 학습 취소 / 학습 상태가 나타납니다.
- 한 번호에서 학습한 신호는 다른 번호에 전혀 영향을 주지 않습니다 — 실제로 "3"번에서 학습을 시작해도 "0"번의 저장된 신호는 그대로 유지되는 것을 확인했습니다.
- 물리 기기의 학습 모드는 하나뿐이므로, 학습이 진행 중인 상태에서 다른 번호의 "새 신호 학습"을 누르면 새 번호가 학습 대상이 되고 이전 번호의 "학습 중" 표시는 자동으로 "대기"로 돌아갑니다.
...
1. 기기 상세 화면 위쪽에서 원하는 번호("버튼 0" ~ "버튼 9")를 선택
2. "새 신호 학습" 버튼을 누름 (학습 상태가 "학습 중"으로 바뀜)
3. 몇 초 안에 기존 리모컨을 기기에 가까이 대고 버튼을 누름 → "저장된 신호"가 "저장됨"으로 바뀌면 성공 (60초 안에 신호가 안 들어오면 자동으로 "대기"로 돌아갑니다)
4. 같은 번호에서 "저장된 신호 재생" 버튼을 누르면 그 번호에 학습된 신호가 재생됨

### CLI에서 (여러 코드를 따로 보관하고 싶을 때)

명령 앞에 `<컴포넌트 id>:`를 붙이면 해당 컴포넌트를 지정할 수 있습니다 (`main`이 "버튼 0", 나머지는 `num1` ~ `num9`가 "버튼 1" ~ "버튼 9"에 대응).
...
2. "그런 다음(then)" 동작에서 이 IR 기기를 선택 → 상단에서 원하는 번호("버튼 0" ~ "버튼 9") 선택 → **"IR 블라스터"** 항목에서 "새 신호 학습" / "학습 취소" / "저장된 신호 재생" 중 원하는 동작 선택 (보통 "저장된 신호 재생"을 쓰게 됩니다)
...
## iOS 앱에서만 기기가 목록에서 사라지는 문제

iOS 사용자들에게서 "기기 목록에서 기기가 사라진다"는 제보가 있었습니다. iOS와 Android는 같은 기기 프레젠테이션(vid)을 받지만, 한쪽에서만 증상이 나타나는 것은 충분히 가능합니다 — iOS 앱이 프레젠테이션 파싱에 더 엄격하고, 무엇보다 **사용자가 앱 캐시를 직접 비울 방법이 iOS에는 없습니다**(Android는 앱 설정에서 캐시 삭제 가능). 이 드라이버는 프로필이 10번, 커스텀 capability가 4세대(`irBlaster` → V2 → V3 → V4)까지 바뀌었기 때문에, 낡은 화면 정의가 캐시에 남아 카드 렌더링에 실패하면 그 기기만 목록에서 빠질 수 있습니다.

점검 결과 실제로 두 가지 문제를 찾아 고쳤습니다.

1. **프로필에 기기 타입(`ocfDeviceType`)이 아예 없었습니다.** 프로필에 `metadata` 블록이 없어서, 플랫폼에 저장된 값이 `vid`/`mnmn`뿐인 것을 확인했습니다(같은 저장소의 `zgir01-driver`는 설정되어 있습니다). 앱이 기기 카드와 아이콘을 만들 때 참조할 기기 타입이 비어 있던 상태입니다. → `RemoteController` / `x.com.st.d.remotecontroller`로 지정했습니다(API 검증 통과).
2. **대시보드 액션이 "선언은 되어 있는데 내용은 비어 있는" 상태였습니다.** 플랫폼이 자동 생성하는 기기 설정(device config)은 대시보드 액션으로 `irBlasterV4`를 가리키는데, 정작 capability 프레젠테이션의 `dashboard.actions`가 `[]`여서 병합 결과가 빈 액션이었습니다. → 표준 `momentary` capability와 똑같은 형태(`displayType: pushButton`)로 `replayLearnedCode`를 대시보드 액션에 넣었습니다. 병합된 프레젠테이션에서 액션이 정상적으로 채워지는 것을 확인했고, 덤으로 기기 목록 카드에서 바로 신호를 재생할 수 있게 됐습니다.

프로필이 바뀌면 새 프레젠테이션(vid)이 생성되므로, 이미 페어링된 기기도 앱이 화면 정의를 새로 받아옵니다. 그래도 목록에 안 보이면 기기를 삭제 후 다시 페어링하거나, iOS 앱을 재설치하면 캐시가 초기화됩니다.

**주의**: iOS 실기기로 재현/검증하지는 못했습니다. 위 두 가지는 "실제로 잘못돼 있던 부분"이라 고칠 가치가 확실하지만, 이것이 제보된 증상의 원인이라고 단정할 수는 없습니다. 재발하면 다음을 순서대로 시도해볼 수 있습니다(아직 적용하지 않음).

- 모든 컴포넌트에 표준 capability(`momentary`) 추가 — 커스텀 capability 화면 생성이 실패해도 앱이 그릴 것이 남습니다. 대신 컴포넌트마다 "모멘터리" 버튼이 하나씩 더 생겨 화면이 지저분해집니다.
- 컴포넌트 10개를 줄이기 — 프레젠테이션 크기를 줄입니다.
- capability를 V5로 새로 만들기 — 이 저장소에서 반복 확인된, 이미 페어링된 기기에 화면 변경을 강제로 전파시키는 방법입니다(다만 기존 루틴이 끊깁니다).
