...
물리 기기 자체는 슬롯이 없어서 한 번에 신호 하나만 기억할 수 있는 칩입니다. 하지만 실제로 코드를 "저장"하는 건 SmartThings 쪽(이 드라이버)이고, 재생할 때마다 그 저장된 데이터를 기기로 다시 보내는 방식이라서, **여러 개의 독립적인 신호를 SmartThings 쪽에서 각각 저장**해두는 게 가능합니다. 이를 위해 `../zgir01-driver`와 동일한 방식으로 **자식 기기(child device)**를 사용합니다.

- 페어링하면 기본으로 메인 기기 포함 **6개**의 독립적인 학습/재생 슬롯이 생깁니다 (메인 1개 + 자식 기기 5개, 목록에 "...1 2" ~ "...1 6"처럼 표시됨). 각 자식 기기의 이름은 SmartThings 앱에서 "TV 전원", "에어컨 켜기"처럼 원하는 대로 바꿀 수 있습니다.
- 기기 설정(메인 기기에서 톱니바퀴 아이콘)의 **"학습 가능 신호 개수"**를 1~10 사이로 바꾸면 자식 기기가 자동으로 더 생깁니다. 단, 개수를 줄여도 이미 만들어진 자식 기기는 자동으로 삭제되지 않습니다 — 필요 없어진 자식 기기는 SmartThings 앱에서 직접 삭제하세요.
- 각 기기(메인이든 자식이든)는 완전히 독립적으로 "새 신호 학습" → "저장된 신호 재생"을 할 수 있습니다. 예를 들어 메인 기기에 "TV 전원"을, 자식 기기 1개에 "에어컨 전원"을 각각 학습시켜두면 서로 덮어쓰지 않고 각자 재생됩니다.

## 사용법

### 앱에서

1. 원하는 기기(메인 또는 자식 중 하나, 예: "TV 전원"으로 이름 바꾼 기기)의 상세 화면에서 "새 신호 학습" 버튼을 누름 (학습 상태가 "학습 중"으로 바뀜)
2. 몇 초 안에 기존 리모컨을 기기에 가까이 대고 버튼을 누름 → "저장된 신호"가 "저장됨"으로 바뀌면 성공
3. 같은 기기에서 "저장된 신호 재생" 버튼을 누르면 그 기기에 학습된 신호가 재생됨 — 다른 기기(슬롯)에 학습된 신호와는 서로 섞이지 않습니다
...
**다중 신호(자식 기기) 지원**: 자식 기기 생성과 명령 라우팅(자식 기기에서 `learn`/`cancelLearn` 호출 시 실제 Zigbee 무선 통신은 항상 부모 기기를 통해 나가고, 학습 상태/코드는 올바르게 그 자식 기기 자신에게 기록되는 것)을 실기기로 확인했습니다. 다만 자식 기기로 실제 리모컨 신호를 학습해서 끝까지 저장되는 전체 사이클(물리 리모컨 필요)까지는 아직 실사용 확인 전입니다 — 메인 기기로는 이미 여러 차례 확인된 것과 동일한 코드 경로를 타므로 정상 동작할 것으로 예상하지만, 이상하면 알려주세요.
