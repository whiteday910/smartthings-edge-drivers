...
**재생 명령이 겹치면 아무것도 송출되지 않던 문제 (수정)**: "앱에서 재생/학습을 누르면 멈추고 안 된다, 루틴으로도 신호가 안 나간다"는 제보를 조사하다 재현한 버그입니다. 재생 명령 두 개가 거의 동시에 들어오면(반응이 늦어 버튼을 다시 누를 때, 루틴 하나에 IR 동작이 여러 개일 때 등) 기기는 두 전송을 큐에 넣고 하나씩 데이터를 요청합니다. 그런데 드라이버는 전송 대기 데이터를 **마지막 하나만** 기억하고 있어서, 앞 전송의 청크 요청에 응답하지 못했습니다. 기기는 그 전송을 약 5초 간격으로 20초 동안 재요청하다 포기하고, 뒤 전송도 같은 이유로 실패합니다. 실기기에서 재생 2개를 한 번에 보내자 **37초 동안 IR이 한 번도 나가지 않았고**, 그 사이 다시 누르면 먹통 시간이 더 늘어납니다. 게다가 기기가 보내는 종료 프레임(`0x04`)의 첫 바이트가 실패(`0x03`)여도 드라이버가 "IR code transmit completed"로 기록해서, 로그로는 실패를 알 수 없었습니다(성공한 전송 12건은 모두 `0x00`).

- ZHA 참조 구현(`ir_msg_to_send[seq]`)처럼 전송 대기 데이터를 seq별로 보관하도록 바꿨습니다(최근 8개).
- `0x04`의 첫 바이트가 0이 아니면 `IR code transmit failed (seq N, status 0xNN)` 경고로 기록합니다.

수정 후 같은 조건(한 요청에 재생 2개)에서 두 전송 모두 상태 `0x00`으로 4.3초 만에 완료되는 것을 실기기로 확인했습니다. 제보 당시 상황을 직접 관찰하지는 못해서 이것이 유일한 원인이라고 단정할 수는 없지만, 증상과 같은 형태로 재현되는 문제였습니다.
