...
- IR 스위치 1~6 (각각 켜짐/꺼짐에 서로 다른 IR 코드를 매핑해서 전송)
  - 기기는 **1개만** 생성되고, 스위치 1~6은 그 기기의 컴포넌트("스위치 1"~"스위치 6")로 상세페이지 안에서 모두 표시됩니다. 자식 기기를 만들지 않습니다.
- 스위치별 IR 코드 학습: 각 스위치 컴포넌트에 "켜짐(ON) 신호 학습" / "꺼짐(OFF) 신호 학습" 버튼과 신호 저장 상태("없음"/"저장됨"/"학습 중") 표시가 상세페이지에 나옵니다 (커스텀 capability `acrosswatch58328.irSlotStudy`).
- 온도 / 습도 / 배터리 센서 (메인 컴포넌트에 표시)
...
- 표준 Tuya EF00(0xEF00) 클러스터의 datapoint(DP) 프로토콜을 사용:
  ...
  | 112 | 배터리 잔량 % | value |
  | 120-131 | 스위치별 on/off 코드 학습 상태 (0=study, 1=registered, 2=unregistered; 스위치 N의 ON 코드=DP 118+2N, OFF 코드=DP 119+2N) | enum |
...
4. 기기는 1개만 생성되고, 상세페이지 상단의 컴포넌트 선택기에서 "스위치 1"~"스위치 6"을 오가며 사용합니다.

이전 버전(자식 기기 방식)에서 업그레이드한 경우: 드라이버 업데이트 후 메인 기기에 스위치 2~6 컴포넌트가 자동으로 생기지만, 이전에 만들어진 자식 기기 5개("... 2"~"... 6")는 남아 있으므로 앱에서 직접 삭제해주세요.

### 5. IR 코드 학습시키기

기존에 쓰던 리모컨(에어컨, TV 등)의 코드를 각 스위치에 매핑하려면:

1. SmartThings 앱에서 기기 열기 → 상단에서 원하는 스위치(1~6) 컴포넌트 선택
2. "켜짐(ON) 신호 학습" 또는 "꺼짐(OFF) 신호 학습" 버튼 누름 → 해당 신호 상태가 "학습 중"으로 바뀜
3. 몇 초 안에 기존 리모컨을 ZG-IR01에 가까이 대고 원하는 버튼을 누름
4. 기기가 코드를 저장하면 상태가 "저장됨"으로 바뀝니다. 이후 스위치 N을 켜거나 끄면 그 코드가 전송됩니다.
5. 재학습하려면 같은 학습 버튼을 다시 누르면 됩니다.
